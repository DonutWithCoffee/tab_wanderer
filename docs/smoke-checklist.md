...
2. Открыть тот же заказ в Склад 3 и убедиться, что панель раскрыта.
3. Доверенно нажать складскую кнопку «Собрать заказ».
4. Наблюдать Warehouse panel и Ozon worker.
5. Повторить быстрый двойной click/Enter только в безопасном тестовом сценарии.
...
- fallback snapshot считается свежим только при изменившемся наборе штрихкодов;
- двойной click не создаёт параллельные automatic sessions;
- automatic request связан с одним `actionId` до финального результата;
