...
состояние, eventJournal и diagnostic/foundation order lookup продолжают обновляться
...
3. Проверить уведомления и диагностический лог.
```
...
## 7. Страница “Отслеживание” / список заказов
...
3. В popup добавить заказ по полному orderId.
4. Проверить список отслеживаемых заказов.
5. Удалить заказ из отслеживаемых.
6. Добавить снова со страницы “Отслеживание”.
7. Попробовать добавить несуществующий заказ формата ####-######.
```
...
несуществующий форматный номер не добавляется и показывает понятный отказ без зависания
```

---

## 8. Отслеживаемые заказы / интервал прямой проверки

Шаги:

```text
1. Открыть Options.
2. Найти настройку “Интервал прямой проверки”.
3. Выставить 5 минут.
4. Закрыть и снова открыть Options.
5. Проверить, что значение сохранилось.
6. Добавить отслеживаемый заказ.
7. Проверить diagnostic header/status: интервал = 5 мин.
8. Вернуть значение 2 минуты перед финальным smoke, если нужна быстрая проверка.
```
...
значение autosave сохраняет настройку без отдельной кнопки
default = 2 минуты
...
## 9. Отслеживаемые заказы / напоминания

Шаги:

```text
1. Открыть страницу “Отслеживание”.
2. Выбрать отслеживаемый заказ без активного pending reminder.
3. Указать дату/время через datetime-local.
4. Добавить короткий note.
5. Нажать “Напомнить”.
6. Перезагрузить страницу “Отслеживание”.
7. Убедиться, что pending reminder отображается у того же заказа.
...
note опционален
у заказа может быть только одно активное pending reminder
...
6. Изменить интервал прямой проверки отслеживаемых заказов.
7. Открыть ссылку на “Отслеживание”.
```
...
интервал прямой проверки сохраняется и отображается в диагностике
```

---

## 11. Диагностический лог
...
## 12. Остановка / перезапуск / восстановление
...
## 13. Warehouse / Ozon barcode binding
