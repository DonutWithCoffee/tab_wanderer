...
Автозапись создаётся только после доверенного нажатия складской кнопки сборки подтверждённого Ozon-заказа. Расширение ждёт свежий успешный post-action snapshot со штрихкодами, затем запускает существующий compare/write/verify flow. Для `regular` и `unknown` Ozon worker автоматически не открывается.
