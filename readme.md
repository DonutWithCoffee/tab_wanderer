...
Текущая разработка: release-prep bugfix 1.0.4.1
Manifest candidate: 1.0.4.1
Tests: 326 pass / 0 fail
```

Версия 1.0.4 опубликована и отмечена тегом `v1.0.4`. После подтверждённого случая записи мультиштрихкода реализован и live-проверен fail-closed bugfix. Для доставки пользователям он оформляется отдельным CWS build `1.0.4.1`, без новых возможностей, permissions или изменений обработки данных.
